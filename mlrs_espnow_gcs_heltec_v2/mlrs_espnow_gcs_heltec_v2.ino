//*******************************************************
// mLRS ESP-NOW GCS Bridge — Heltec WiFi Kit 32 v1/v2/v3
...
int       telem_bat_pct     = -1;
int       telem_rssi_pct    = -1;   // 0-100%, -1 = invalid
...
// Firmware autodetect (from HEARTBEAT autopilot field)
// autopilot: 3 = ArduPilot, 12 = iNav
...
const char* get_mode_str(uint32_t mode) {
    ...
    return get_mode_str_ardu(mode);  // default to ArduPilot
}
    ...
    switch (pkt.msg_id) {
        ...
        case MAVLINK_MSG_ID_RADIO_STATUS:
            if (pkt.payload_len >= 5) {
                uint8_t raw = p[4];
                // mLRS reports rssi as 0-100 directly (not 0-254 like SiK)
        ...
        case MAVLINK_MSG_ID_HEARTBEAT:
            if (pkt.payload_len >= 7) {
                ...
                // Autodetect firmware from autopilot field (once)
                ...
                // Accept fixed wing(1), multirotor(2), tricopter(13),
                // VTOL(19), and iNav generic(3) — reject GCS(6)
...
void oled_status(const char* l1, const char* l2 = "") {
    display.clearDisplay();
    display.setCursor(0, 0);  display.println("mLRS ESP-NOW GCS");
...
void oled_telem() {
    ...
    // Line 0: Mode + armed + sats + fw label
    ...
    // mirrors GS in airspeed and copters have no meaningful airspeed, so
    // we just show GS in those cases.
...
void setup() {
    ...
    oled_init();
    oled_status("Initializing...", "mLRS ESP-NOW GCS");
    delay(1500);
