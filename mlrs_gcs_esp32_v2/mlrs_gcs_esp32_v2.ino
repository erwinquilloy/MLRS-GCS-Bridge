//*******************************************************
// mLRS GCS Bridge - generic ESP32 (no OLED)
//
// Port of mlrs_gcs_heltec_v2 with all SSD1306/OLED code
// removed. Status and telemetry are printed to the USB
// serial monitor instead. Otherwise functionally identical:
// ESP-NOW + WiFi UDP link layers, runtime-toggleable via
// the onboard BOOT/PRG button (GPIO0), mode persisted to
// NVS, MAVLink HEARTBEAT presence ping in UDP mode.
//
// Default mode on fresh flash: WiFi UDP.
//
// Forwards MAVLink to MFD Mini Crossbow via Serial2 TX
// (GPIO17 by default; see TX_PIN).
//
// No external libraries required - core only.
//
// Board: Tools -> Board -> ESP32 Arduino -> ESP32 Dev Module
//        (or whichever generic ESP32 dev board you're using)
...
#define PRG_PIN         0      // Onboard BOOT/PRG button (active LOW)
#define SPLASH_MS       2500   // Hold PRG this long during boot to toggle mode
...
//-------------------------------------------------------
// Serial status print (replaces OLED HUD)
//-------------------------------------------------------

void serial_status(const char* l1, const char* l2 = "") {
    Serial.printf("[mLRS %s] %s", mode_str(current_mode), l1);
...
void setup_udp(void) {
    ...
    // dedicated UBEC for the ESP32 if that's an issue.
    esp_wifi_set_protocol(WIFI_IF_STA, WIFI_PROTOCOL_11B);
    WiFi.setTxPower(WIFI_POWER_19_5dBm);
...
//-------------------------------------------------------
// Boot splash + mode toggle (Serial only)
//
// No OLED, so the countdown prints to Serial. Hold the
// BOOT/PRG button (GPIO0) for the entire window to toggle.
//-------------------------------------------------------
...
void splash_and_maybe_toggle() {
    ...
    Serial.println();
    Serial.println("---- mLRS GCS Bridge (ESP32) ----");
    Serial.printf("Mode: %s\n", mode_str(current_mode));
    Serial.println("Hold BOOT/PRG for whole countdown to swap mode.");
    ...
    while (millis() - t0 < SPLASH_MS) {
        ...
        if (now - last_print >= 500) {
            last_print = now;
            unsigned long remaining = SPLASH_MS - (now - t0);
            Serial.printf("Boot in %lus%s\n",
                (remaining + 999) / 1000,
                pressed ? "  [PRG held]" : "");
        }
        delay(20);
...
void setup() {
    ...
    // TX only - if your hardware has a back-channel from the Crossbow you
    // can wire a real RX pin here and uplink will start flowing.
